...
// --- WIDGET 0: OPORTUNIDAD DESTACADA (NUEVO) ---
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
                ...
                <h3 className="text-3xl font-bold mb-1">
                    ${value.toLocaleString()} <span className="text-indigo-200 text-lg font-normal">en ventas potenciales</span>
                </h3>
                <p className="text-indigo-100 max-w-lg">
                    Detectamos <strong>5 clientes</strong> en tu red buscando productos que no tienes en stock. ¡Consíguelos y cierra la venta hoy!
                </p>
            </div>
            ...
            <TableRow key={req.id} className="hover:bg-indigo-50/30 group">
              ...
              <TableCell>
                  <div className="flex items-center gap-1 font-bold text-emerald-600 bg-emerald-50 w-fit px-2 py-1 rounded-md">
                    <DollarSign className="w-3 h-3" /> {req.potential?.toLocaleString()}
                  </div>
              </TableCell>
              <TableCell className="text-slate-600">{req.cliente}</TableCell>
              <TableCell className="text-right">
                <Button size="sm" className="h-8 text-xs bg-slate-900 hover:bg-indigo-600 text-white shadow-md transition-all">
                  Capturar Venta
                </Button>
              </TableCell>
            </TableRow>
...
// --- WIDGET 2: DEMO SEARCH (IGUAL PERO MAS LIMPIO) ---
        ...
        {data.map((term, i) => (
          <div key={i} className="p-4 hover:bg-slate-50 transition-colors">
            <div className="flex items-center justify-between mb-2">
              ...
              <Badge variant="outline" className="text-slate-500 font-normal">{term.count} búsquedas</Badge>
            </div>
...
// --- WIDGET 3: FORECAST (Sin cambios funcionales, solo estilo) ---
