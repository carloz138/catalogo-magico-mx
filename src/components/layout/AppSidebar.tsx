    ...
    items: [
      {
        ...
        path: "/upload",
        icon: Upload, // ✅ Icono corregido del mismo tamaño
        badge: "Nuevo",
...
export function AppSidebar() {
    ...
    if (hasChildren) {
      return (
        <SidebarMenuItem key={item.title}>
          <SidebarMenuButton className="w-full group hover:bg-slate-100 transition-colors">
            <item.icon className="w-5 h-5 text-slate-600 group-hover:text-slate-800" /> {/* ✅ Tamaño consistente */}
            <span className="text-slate-700 group-hover:text-slate-900 font-medium">{item.title}</span>
            ...
            {item.items?.map((subItem) => (
              <SidebarMenuSubItem key={subItem.title}>
                ...
                >
                  ...
                  >
                    <subItem.icon className="w-5 h-5 flex-shrink-0" /> {/* ✅ Tamaño consistente w-5 h-5 */}
                    <span className="font-medium">{subItem.title}</span>
                    {subItem.badge && (
                      <Badge
                        className={`text-xs ml-auto border ${
    ...
    return (
      <SidebarMenuItem key={item.title}>
        <SidebarMenuButton asChild isActive={isActive}>
          ...
          >
            <item.icon className="w-5 h-5 flex-shrink-0" /> {/* ✅ Tamaño consistente */}
  ...
  return (
    <Sidebar className="border-r border-slate-200 bg-white"> {/* ✅ Fondo sólido blanco */}
