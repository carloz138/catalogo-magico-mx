...
import { TrendingUp, BrainCircuit, Loader2, CalendarDays } from "lucide-react";
...
export const DemandForecastWidget = ({ catalogId }: { catalogId: string | null }) => {
  // 1. GATING: Solo Enterprise (o la feature que definas para el plan más alto)
  // Si no tienes una feature 'predictivo', usa 'recomendaciones' y valida el nombre del plan,
  // o idealmente agrega 'predictivo' a tu useFeatureAccess.
  ...
  const fetchHistoryAndPredict = async () => {
    setLoading(true);
    try {
      // 1. Obtenemos el término más buscado de los últimos 30 días para analizar SU tendencia
      // (Para simplificar la demo, analizamos el "Top Mover")
      ...
      logs.filter(l => l.search_term.toLowerCase().trim() === winnerTerm).forEach(l => {
      ...
      // Calculamos la pendiente (m) y la intersección (b) de y = mx + b
    ...
    return (
      <div className="h-full min-h-[400px]">
        {React.cloneElement(UpsellComponent as React.ReactElement, {
            ...
            description: "Adelántate al futuro. Nuestra IA analiza tendencias y te dice qué se venderá la próxima semana."
        })}
      </div>
    );
  }
      ...
      <CardHeader>
        <div className="flex justify-between items-start">
            <div>
                ...
                <CardDescription>
                    Basado en el comportamiento de búsqueda de los últimos 30 días.
                </CardDescription>
            </div>
      ...
      <CardContent className="h-[350px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            ...
            {/* Línea divisoria Hoy */}
            <ReferenceLine x={format(new Date(), "dd MMM", { locale: es })} stroke="red" strokeDasharray="3 3" label="Hoy" />
          </AreaChart>
        </ResponsiveContainer>
