    ...
    const steps: TimelineStep[] = [
      {
        ...
        desc: "Tu pedido ha sido registrado.",
        icon: Clock,
        match: ["draft", "sent"],
      },
      {
      ...
      },
      {
        key: "shipped",
        label: "En Camino",
        desc: "Tu pedido salió a ruta.",
        icon: Truck,
        match: ["shipped", "in_transit"],
      },
              ...
              <div className="relative z-10 flex justify-between items-start">
                <div>
                  <p className="text-indigo-200 text-sm font-medium">
                    Pedido #{order.quote_number || order.id.slice(0, 6)}
                  </p>
                  <h2 className="text-3xl font-bold mt-1">
                    ${(order.total_amount / 100).toFixed(2)}{" "}
                    <span className="text-base font-normal text-indigo-200">{order.currency}</span>
                  </h2>
                </div>
                    ...
                    {steps.map((step, index) => {
                      ...
                      return (
                        <div key={step.key} className="relative pl-8">
                          {/* Bolita del Timeline */}
                          <div
                            className={`absolute -left-[23px] top-0 w-10 h-10 rounded-full border-4 flex items-center justify-center transition-colors duration-300 z-10 ${
                              isActive
