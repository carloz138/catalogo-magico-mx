...
const ImageReview = () => {
  ...
  const loadProcessedImages = async () => {
    ...
    try {
      ...
      const convertedImages = (productsData || [])
        ...
        .filter((img) => img.status === "completed" && (img.processed_url || img.hd_url));
  ...
  const handleForceRefresh = async () => {
    setLoading(true);
    await Promise.all([loadProcessedImages(), loadUsageValidation(), calculateCreditsConsumed()]);
  };
        ...
        <CardContent className="p-3">
          ...
          <div className="md:hidden space-y-2">
            <div className="flex items-center justify-between">
              <h4 className="font-semibold text-blue-900 text-sm">{usageValidation.currentPlan}</h4>
          ...
          <div className="hidden md:flex items-center justify-between">
            <div className="flex items-center gap-6">
              <div>
                <h4 className="font-semibold text-blue-900">{usageValidation.currentPlan}</h4>
                <div className="text-sm text-blue-700">{usageValidation.remainingBgCredits} créditos disponibles</div>
              </div>
    ...
    <div className="flex items-center gap-2">
      ...
      {selectedImages.length > 0 && (
        <div className="flex items-center gap-1 md:gap-2">
          <Button onClick={handleSaveAndCreateCatalog} disabled={saving} size="sm">
            {saving ? (
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Imágenes Procesadas</h1>
          <p className="text-gray-600">Revisa y selecciona imágenes para crear catálogos</p>
        </div>
      ...
      {images.length === 0 ? (
        <Card>
          <CardContent className="text-center py-8 md:py-12">
            ...
            <h3 className="text-base md:text-lg font-medium text-gray-900 mb-2">No hay imágenes procesadas</h3>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 md:gap-4">
                  ...
                  <div className="text-xs md:text-sm text-gray-600">
                    ...
                    {selectedImages.length > 0 && (
                      <Badge className="ml-2 bg-green-100 text-green-800 text-xs">Listas</Badge>
                    )}
                  </div>
                </div>
            ...
            {images.map((image) => (
              <Card key={image.id} className="overflow-hidden hover:shadow-md transition-shadow group">
                ...
                <CardContent className="p-2 md:p-3">
                  <h3 className="font-medium text-xs md:text-sm truncate mb-1">{image.product_name}</h3>

                  <div className="flex items-center justify-between text-xs">
                    {image.price_retail && (
                      <span className="font-bold text-primary">${(image.price_retail / 100).toFixed(0)}</span>
                    )}
                    {image.category && (
