...
// ESTADO: FIX_V8 (REVERTIDO A URL CANÓNICA: ?token=UUID)
// ==========================================
import { createClient } from "jsr:@supabase/supabase-js@2.49.8";
// 🚫 ELIMINAMOS EL IMPORT DE UUID: Usamos crypto.randomUUID()
// 1. FINGERPRINT: Versión de replicación
const DEPLOY_VERSION = "2025.11.19_v8.0_CANONICAL_URL_REVERT";
const corsHeaders = {
  ...
  try {
    ...
    if (isDistributable) {
      ...
      const { data: existingReplica } = await supabaseAdmin
        ...
        .maybeSingle();
      if (existingReplica) {
        // ⬅️ REVERTIDO: Se usa ?token= para seguir la documentación canónica
        ...
        // 1.3. Construir el Magic Link (Fase C) - ⬅️ REVERTIDO: Se usa ?token= para la doc.
...
function generateAcceptedEmailTemplate(quote, linkToSend, customerName, isDistributable, userData) {
  ...
  const totalInCents = items.reduce((sum, item) => sum + item.subtotal, 0);
